...
class FacialLoginSystem:
    def __init__(self):
        ...
        self.lock_grace_period = 5
        self.last_authorized_seen_time = None
        ...
        self.AUTHORIZED_COLOR = (0, 255, 0)
        self.UNAUTHORIZED_COLOR = (0, 0, 255)
        self.SPOOF_COLOR = (255, 0, 255)

        self.conn = sqlite3.connect('SDP.db')
        self.cursor = self.conn.cursor()

        self.authenticate_user() ### Added
        self.select_room()
...
###
    ...
    def authenticate_user(self):
        ...
        while True:
            ...
            self.cursor.execute("SELECT usnum FROM Users WHERE username = ? AND password = ?", (username, password))
            result = self.cursor.fetchone()

            if result:
                self.logged_in_usnum = result[0]  # Store for cross-reference later
    ...
    def detect_blink(self, full_frame, face_location, face_id):
        if face_id not in self.blink_counters:
            self.blink_counters[face_id] = {'counter': 0, 'total': 0}
    ...
    def check_time_access(self, role):
        ...
        self.cursor.execute(
            'SELECT * FROM TimeRules WHERE role_name=? AND day=? AND ? >= start_time',
        ...
        if self.can_login(person_id):
            ...
            if has_access:
                ...
                if self.lock_state != "unlocked":
                    print(f"Access granted for {person_id}. Unlocking the door...")
                    self.kwikset.unlock()
                    self.lock_state = "unlocked"
                    self.restart() ### ADDED THIS
    ...
    def detect_faces(self, frame):
        ...
        for encodeFace, face_loc in zip(encodeCurFrame, original_face_locations):
            ...
            if True in matches:
                ...
                state = self.user_states.get(person_id, {})
                last_blink = state.get("last_blink_time")

                if is_real_face:
                    has_access = self.process_login(person_id)
                    self.user_states[person_id]['last_blink_time'] = now
                else:
                    if last_blink and (now - last_blink).total_seconds() < self.blink_valid_duration:
                        has_access = state.get("has_access", False)
                    else:
                        has_access = False

                self.draw_face_box(frame, face_loc, has_access, person_id, is_spoof=not is_real_face)
            else:
        ...
        for person_id in self.user_states:
            if self.user_states[person_id]['is_visible']:
                self.blink_counters[person_id]['total'] = 0
    ...
    def draw_face_box(self, frame, location, is_authorized, person_id=None, is_spoof=False):
        ...
        if face_id in self.blink_counters:
            ...
            cv2.putText(frame, blink_text, (left, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    ...
    def run(self):
        ...
        while True:
            ...
            authorized_visible = any(
                state.get("has_access") and state.get("is_visible")
                for state in self.user_states.values()
            )

            now = time.time()
            if authorized_visible:
                self.last_authorized_seen_time = now
                if self.lock_state != "unlocked":
                    print("Authorized face detected. Unlocking...")
                    self.kwikset.unlock()
                    self.lock_state = "unlocked"
                    self.restart ### ADDED THIS
                    # time.sleep(10)
                ...
                if self.last_authorized_seen_time and (now - self.last_authorized_seen_time) > self.lock_grace_period:
                    if self.lock_state != "locked":
                        print("No authorized face detected for a while. Locking...")
                        self.kwikset.lock()
                        self.lock_state = "locked"

...
class FacialLoginSystem:
    def __init__(self):
        ...
        # Database connection
        self.conn = sqlite3.connect(r"C:\Users\mendo\AppData\Local\Programs\Python\SDP 2024-2025\SDP Facial Recognition\SDP.db")
    ...
    def load_encodings(self):
        ...
        with open(r'C:\Users\mendo\AppData\Local\Programs\Python\SDP 2024-2025\SDP Facial Recognition\EncodeFile.p', 'rb') as file:
    ...
    def verify_login(self):
     """Verify login by checking username and password."""
     username = input("Enter your username: ").strip()
     password = input("Enter your password: ")

     self.cursor.execute("""SELECT users.usnum, users.password, UsID.UsID FROM users JOIN UsID ON users.usnum = UsID.usnum WHERE users.username = ?""", (username,))
     result = self.cursor.fetchone()

     if result:
        usnum, stored_password, person_id = result #person_id corresponds to UsID from the UsID

        if stored_password == password:
            print("Login successful!")
            return usnum # Return the associated person ID
        else:
    ...
    def validate_user_face(self, username, detected_person_id):
     """Ensure the entered username matches the detected face."""
     self.cursor.execute("""SELECT Us.ID.UsID FROM users JOIN UsID ON users.usnum = UsID.usnum WHERE users.username = ?""", (username,))
     result = self.cursor.fetchone()

     if result:
        correct_person_id = result[0]
        if correct_person_id == detected_person_id:
            print(f"Face verification successful for {username}.")
            return True
        else:
            print(f"Face verification failed: {username} does not match detected face ID {detected_person_id}.")
            return False
     else:
    ...
    # def rehash_user_passwords(self):
    #     """Rehash existing passwords in the database."""
    ...
    #     self.conn.commit()
    #     print("Passwords have been rehashed successfully!")
    ...
    def run(self):
        ...
        while True:
            ...
            #Check if the detected person matches the logged in username
            detected_person_id = next(iter(self.user_states), None) #Get first detected face
            if detected_person_id and not self.validate_user_face(username, detected_person_id):
                print("Mismatch between username and detected face. Access denied.")
                break
